...
export default class Home extends Component {
    ...
    return (
      <div>
        <div style={{textAlign: 'center'}}>
          ...
          {things.map((thing, index) => <div key={index}>{things}</div>)}
          <div>{randomNumber}</div>
          <div>{randomNumber} is {`${randomNumber > 50 ? 'larger than fifty' : 'not larger than fifty'}`}</div>
          <div>{randomWords}</div>
        </div>
        <div>
          <img style={{width: '300px'}} src={gundam} />
        </div>
        <div>
          <img style={{width: '300px'}} src={smartRobots} />
        </div>
        <div>
        <img style={{width: '300px' }} src={robot} />
        </div>
      </div>
    )
  }
  ...
  onAddRandomWords = () =>  {
    ...
    for (let i = 0; i < wordLength; i++) {
      randomWords += alphabet[Math.floor(Math.random() * (alphabet.length - 1))]
    }
